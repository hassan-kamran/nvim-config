...
return {
  -- Core DAP (Debug Adapter Protocol) plugin
  {
    ...
    config = function()
      ...
      -- Key mappings for debugging - fixed to avoid prefix overlaps
      ...
      vim.keymap.set('n', '<leader>dn', dap.step_over, { desc = 'Debug: Step Over' }) -- Changed from do to dn
  ...
  -- Python-specific DAP extensions
  {
    ...
    config = function()
      ...
      -- Test mappings with fixed prefixes to avoid overlaps
      ...
      end, { desc = 'Debug-Python: Debug Selection' })
    end,
  },
